...
class AxisTransformer(object):
   ...
   def get_implicit_permute_order(self, layer_type, input_rank, src_buffer_name, target_buffer_name):
      ...
      next_src_axes, next_src_trackable = self._src_layer_ordered_axes.get_input_axis_order(layer_type, input_rank+1)
      ...
      if curr_target_trackable: # Current state
         if next_target_trackable: # Next state
            ...
            curr_src_axes_minus_batch = curr_src_axes[1:]
            comp = [i for i, j in zip(curr_src_axes_minus_batch, curr_target_axes) if i == j]
            if len(comp) == len(curr_src_axes_minus_batch):
               if AxisAnnotation.ANY not in next_target_axes:
                  ...
                  permute_order = self.compute_permute_order(next_src_axes[1:], next_target_axes)
            ...
            permute_order = self.compute_permute_order(curr_target_axes, curr_src_axes[1:])
   ...
   def get_explicit_permute_order(self, layer_type, input_rank, src_buffer_name, target_buffer_name, src_permute_order):
      ...
      if curr_target_trackable:
         ...
         permute_order = self.compute_permute_order(curr_target_axes, next_src_axis_order[1:])
