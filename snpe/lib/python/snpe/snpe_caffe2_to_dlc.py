...
#//  @@
#//
#//  Copyright 2017-2018 Qualcomm Technologies, Inc. All rights reserved.
        ...
        if len(self.input_dims) > 0 and not self.enable_preprocessing:
            ...
            for data_name, dims in self.input_dims.iteritems():
                ...
                if data_name in self.reorder_list:
                    ...
                    self.model.set_input_order(data_name, [1,2,0])
        ...
        for op in net.op:
            ...
            for index,output in enumerate(op.output):
                if output in self.reorder_list:
                    ...
                    self.model.set_output_order_by_index(str(op.name), index, [2,0,1])
...
#                code_to_message.get_debugging_message('DEBUG_CAFFE2_CONVERT_PROCESS_OUT_DIMS')(self.get_layer_name(op), op.type, str(self.get_output_dims(op))))
    ...
    def do_post_layer_work(self, op):
        ...
        for idx in range(len(src_output_names)):
            ...
            #      get away with +1
            self._axis_transformer.update_src_axis_order(str(op.type), len(output_dims[idx])+1, src_output_names[idx],
    ...
    def add_prelu_layer(self, op):
        ...
        if len(bias) == 1:
            ...
            input_dims = self.get_input_dims(op)
            self.logger.debug(
                code_to_message.get_debugging_message('DEBUG_CAFFE2_CONVERT_RESIZE_BIAS')(str(input_dims[0][2])))
            for i in xrange(input_dims[0][2]-1):
                bias.append(bias[0])
    ...
    def add_slice_op(self, op):
        ...
        if len(start) != 3:
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_SLICE_ONLY_SUPPORTED_FOR_3_DIMS_DATA'))
    ...
    def add_split_op(self, op):
        ...
        if axis == 0:
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_NO_SUPPORT_BATCH_WISE_SLICING'))
    ...
    def add_tile_layer(self, op):
        ...
        if len(input_dim) == 1:
            ...
            raise ValueError(code_to_message.get_message('ERROR_CAFFE2_TILE_AXIS_NOT_SUPPORTED')(layer.name, caffe2_axis))
    ...
    def setup_preprocessing(self, op):
        ...
        data_dims = map(int, [net_dims[1], net_dims[2], net_dims[0]])
        id_ = self.model.add_data_layer(data_name, data_dims, self.encoding, "bgr", "default")
        self._axis_transformer.update_src_axis_order('data', len(net_dims)+1, data_name, len(net_dims)+1)
        self._axis_transformer.update_target_axis_order('data', len(data_dims), data_name, len(data_dims))
        self.save_axis_order(data_name)
        self.layer_id_map[data_name] = id_
        self.logger.debug(
        ...
        if  crop_size != 0 and (crop_size != net_dims[0] or crop_size != net_dims[1]):
            if crop_size > net_dims[0] or crop_size > net_dims[1]:
                errs = code_to_message.get_message('ERROR_CAFFE2_CROP_SIZE_LARGER_THAN_INPUT_DIMS')(crop_size, net_dims[0:2])
                raise ValueError(errs)
            offset_y = (net_dims[0]-crop_size)//2
            offset_x = (net_dims[1]-crop_size)//2
            offsets = [offset_y,offset_x,0]
            output_dim = [crop_size,crop_size,data_dims[2]]
            ...
            self._axis_transformer.update_src_axis_order('CROP', len(net_dims)+1, crop_layer_name, len(net_dims)+1, last_layer_name)
            ...
            last_layer_name = crop_layer_name
            crop_dim = [crop_size, crop_size, net_dims[2]]
        else:
            crop_dim = net_dims[:]
        ...
        if mean_data is not None:
            ...
            self._axis_transformer.update_src_axis_order('SUBTRACTMEAN', len(net_dims)+1, subtract_mean_layer_name, len(net_dims)+1, last_layer_name)
