        ...
        with session.graph.as_default():
            ...
            for name, shape, input_type in zip(input_nodes_names, input_nodes_shapes, input_nodes_types):
                ...
                if len(batched_shape) == 0:
                    try:
                        batched_shape = [1] + map(int, shape.split(','))
                    except ValueError:
                        raise ConverterError(code_to_message.get_message('ERROR_TF_INVALID_INPUT_DIMS')(shape))

                try:
                    input_tensor.set_shape(batched_shape)
                except ValueError, e:
                    raise ConverterError(code_to_message.get_message('ERROR_TF_INVALID_INPUT_DIMS')(e.message))
